jQuery(function () {
    ...
    function addFile(file) {
        ...
        var imgLeft = index * (thumbnailWidth + 30);
        var imgTop = 0;
        ...
        if (imgLeft > 900) {
            ...
            imgLeft = (index % 4) * (thumbnailWidth + 30)
        }
    ...
    function setState(val) {
        ...
        switch (state) {
            ...
            case 'finish':
                stats = uploader.getStats();
                if (stats.successNum) {//success
                    // console.log(title,owner,type,profile,fmname,zpname)
                    $.ajax({
                        type: "post",
                        url: "http://localhost/pc/index.php/Home/Upload/works",
                        data: {
                            title: title,
                            owner: owner,
                            type: type,
                        ...
                        success: function (result) {
                            if (result.status == '1') {
                                layer.closeAll();
                                layer.msg('上传完成！', function () {
                                    setTimeout(" window.location.reload();window.scrollTo(0,0);", 1000)
                                });
    ...
    uploader.on('uploadSuccess', function (file, res) {
        zpname.push(res.file.name.replace('/', '_'));
    });

    uploader.on('uploadError', function (file) {
        console.log(file.id + '上传出错');
        layer.msg(file.id + '上传出错');
        return false;
    });
    ...
    $upload.on('click', function () {
        title = $('.inp').val();//标题
        owner = $('.owner').text();//所有者
        type = $('.type').text();//类型
        profile = $('#profile').val();//描述
        if (title == '') {
            layer.msg('作品标题不能为空！');
            return false;
        ...
        //封面
        $.ajaxFileUpload({
            url: 'http://localhost/pc/index.php/Home/Upload/upload',
            type: 'post',
            fileElementId: 'file', // 上传文件的id、name属性名
            dataType: 'json',
            type: 'json',
            success: function (data) {
                fmname = JSON.parse(data).file.name.replace('/', '_');
                uploader.upload();
            }, error: function (data) {
